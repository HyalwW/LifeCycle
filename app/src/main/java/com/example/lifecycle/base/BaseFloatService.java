...
public abstract class BaseFloatService<DB extends ViewDataBinding> extends Service {
    ...
    private void handleMove() {
        if (Build.MANUFACTURER.equals("QUALCOMM")) {
            Configuration configuration = this.getResources().getConfiguration();
            int ori = configuration.orientation;
            if (ori == Configuration.ORIENTATION_LANDSCAPE) {
//                Log.e(TAG, "当前是横屏状态");
                if (params.y <= 2 * statusBarHeight) {
                    params.y = 2 * statusBarHeight;
                }
            } else if (ori == Configuration.ORIENTATION_PORTRAIT) {
//                Log.e(TAG, "当前是竖屏状态");
                if (params.y <= 3 * statusBarHeight + 20) {
                    params.y = 3 * statusBarHeight + 20;
                }
            }
            if (params.y >= displayHeight - navigationBarHeight - BOTTOM_DISTANCE) {
                params.y = displayHeight - navigationBarHeight - BOTTOM_DISTANCE;
            }
        } else {
